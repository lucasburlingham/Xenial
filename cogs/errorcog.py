...
class Error(commands.Cog):
    ...
    async def on_command_error(self,ctx: commands.Context,error):
        ...
        if isinstance(error, commands.CommandNotFound):
            embed = discord.Embed(description=":x: │ The command you typed does not exist...... maybe run the command **>!help**, which will give you a bunch of commands.",colour=discord.Colour.purple())
        ...
        if not (ctx.command.name, type(error)) in ignore_command_errors:
            if isinstance(error, commands.MissingRequiredArgument):
                embed = discord.Embed(description=f":x: │ What you typed was not complete. Maybe add some arguments?",colour=discord.Colour.purple())
                return await ctx.send(embed=embed)
